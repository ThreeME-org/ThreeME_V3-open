  ...
  for (frmt in formmat_img){
    ...
    data_plot <- read_xls(str_c(user_path,"ThreeME_V3/data/France/scenarii.xls"), "calibration",range = "B2:D19", col_names = TRUE) %>% 
      cbind(label_sectors,.) %>% 
      `colnames<-`(c("Products", "Households consumption", "Investments", "Exports")) %>% gather( key = shock, value = value, - Products)
  
    pal_calib <- c("#d3d3d3", "#808080", "#2B2B2B")
    ...
    plot_g <- ggplot() +
      ...
      scale_color_manual(values =  pal_calib, aesthetics = "fill") +
