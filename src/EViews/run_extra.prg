...
subroutine standard_shock(string %shock)
 ...
 ' REDIS_CT_RRSC = 1 - REDIS_CT_LS
  ...
  for %c {%list_com}
    for %s {%list_sec}
      R2_CI_CO2_{%c}_{%s} = R2_CI_CO2_{%c}_{%s} + 0.01 * @elem(GDP/ (EMS_CI_CO2+EMS_MAT_CO2+EMS_Y_CO2+EMS_CH_CO2), %baseyear) 
    next
  next

  for %s {%list_sec}
    R2_MAT_CO2_{%s} = R2_MAT_CO2_{%s} + 0.01 * @elem(GDP/ (EMS_CI_CO2+EMS_MAT_CO2+EMS_Y_CO2+EMS_CH_CO2), %baseyear) 
  next

  for %s {%list_sec}
    R2_Y_CO2_{%s} = R2_Y_CO2_{%s} + 0.01 * @elem(GDP/ (EMS_CI_CO2+EMS_MAT_CO2+EMS_Y_CO2+EMS_CH_CO2), %baseyear) 
  next


  for %c {%list_com}
    R2_CH_CO2_{%c} = R2_CH_CO2_{%c} + 0.01 * @elem(GDP/ (EMS_CI_CO2+EMS_MAT_CO2+EMS_Y_CO2+EMS_CH_CO2), %baseyear) 
  next
...
subroutine standard_outputs(string %scenario, string %grp_name, string %index)
  ...
  smpl {%baseyear}+1 {%baseyear}+1 {%baseyear}+2 {%baseyear}+2 {%baseyear}+3 {%baseyear}+3 {%baseyear}+5 {%baseyear}+5 {%baseyear}+10 {%baseyear}+10 {%baseyear}+50 {%baseyear}+50
