...
subroutine run_scenario(string %scenario_name)
  ...
  if %scenario_name = "baseline" then
    ...
    'GDP and population
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "baseline")
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "cap_efficiency")
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "Electricity_mix")
    'call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "prices_PM_BL")
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "GR_PROG_BASE_E")
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "GR_CI_BASE")
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "GR_PROG_HOUS_BASE")
    'call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "shadowprice_BL")
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "prices_3")
    call load_excel("NLD", "scenarii_ADAPT_elas0_1_0_1", "shadowprice_3")
  ...
  group all_vars *
  %var_names = all_vars.@members

	call export_opera(%scenario_name) 

    return
  endif

if %scenario_name = "scenario_ADAPT" then
  ' Create a new scenario that can be compared with the baseline
   {%modelname}.scenario(n, a=2) {%scenario_name}
    ...
    smpl {%baseyear} @last
    call load_excel("NLD", "scenarii_ADAPT", "baseline")
    call load_excel("NLD", "scenarii_ADAPT", "cap_efficiency")
    call load_excel("NLD", "scenarii_ADAPT", "Electricity_mix")
    call load_excel("NLD", "scenarii_ADAPT", "GR_PROG_BASE_E")
    call load_excel("NLD", "scenarii_ADAPT", "GR_CI_BASE")
    call load_excel("NLD", "scenarii_ADAPT", "GR_PROG_HOUS_BASE")
    call load_excel("NLD", "scenarii_ADAPT", "prices_1")
    'call load_excel("NLD", "scenarii_ADAPT", "PCGAS_1")
...
' ==============    OUTPUTS    ===============================================
' ============================================================================
subroutine export_opera(string %scenario_name) 
...
if %scenario_name = "baseline" then
	...
	%export_opera = %export_opera + %var_names

  'for %s {%list_sec}
  '    %export_opera = %export_opera + " f_e_"+%s+"_0"
  'next

  'for %c {%list_com}
  'for %s {%list_sec}
'	if @isobject("CI_"+%c+"_"+%s+"_0") then
'   	%export_opera = %export_opera + " CI_"+%c+"_"+%s+"_0"
'	endif
'  next
'next

  for %c {%list_com}
      %export_dom_gas_use = %export_dom_gas_use + " qd_"+%c+"_0"
  next


    '%export_dom_gas_use = %export_com_gas_use + " qd_cgas_0"


  ' Create data group sheets 
    group baseline {%export_opera}

   group baseline_dom_gas_use {%export_dom_gas_use}

  ' Export to excel  
    call savetoexcel("baseline", "export_opera.xlsx", %scenario_name, "YES")
'Save only trajectory with domestic gas-use to another excel 
    call savetoexcel("baseline_dom_gas_use", "export_opera_dom_gas_use.xls", %scenario_name, "NO")
  ...
  for %s {%list_sec}
      %export_opera = %export_opera + " Y_"+%s+"_2"
  next

  for %s {%list_sec}
      %export_opera = %export_opera + " va_"+%s+"_2"
  next

  for %s {%list_sec}
      %export_opera = %export_opera + " f_l_"+%s+"_2"
  next

  for %s {%list_sec}
      %export_opera = %export_opera + " f_k_"+%s+"_2"
  next

  for %s {%list_sec}
      %export_opera = %export_opera + " f_e_"+%s+"_2"
  next

  for %s {%list_sec}
      %export_opera = %export_opera + " f_mat_"+%s+"_2"
  next

  for %c {%list_com}
	if @isobject("pch_"+%c+"_0") then
   	%export_opera = %export_opera + " pch_"+%c+"_2"
	endif
  next

  for %c {%list_com}
	if @isobject("ch_"+%c+"_0") then
   	%export_opera = %export_opera + " ch_"+%c+"_2"
	endif
  next

  for %c {%list_com}
      %export_opera = %export_opera + " ci_"+%c+"_2"
  next

  for %c {%list_com}
	if @isobject("pyqs_"+%c+"_0") then
   	%export_opera = %export_opera + " pyqs_"+%c+"_2"
	endif
  next

  for %c {%list_com}
	if @isobject("ci_toe_"+%c+"_0") then
   	%export_opera = %export_opera + " ci_toe_"+%c+"_2"
	endif
  next

  for %c {%list_com}
	if @isobject("ch_toe_"+%c+"_0") then
   	%export_opera = %export_opera + " ch_toe_"+%c+"_2"
	endif
  next

  for %s {%list_sec}
	if @isobject("EMS_CI_CO2_"+%s+"_0") then
   	%export_opera = %export_opera + " EMS_CI_CO2_"+%s+"_2"
	endif
  next

  for %s {%list_sec}
	if @isobject("CI_toe_"+%s+"_2") then
   	%export_opera = %export_opera + " CI_TOE_"+%s+"_2"
	endif
  next

  for %s {%list_sec}
	if @isobject("C_K_"+%s+"_2") then
   	%export_opera = %export_opera + " C_K_"+%s+"_2"
	endif
  next

  for %c {%list_com}
  for %s {%list_sec}
	if @isobject("CI_toe_"+%c+"_"+%s+"_2") then
   	%export_opera = %export_opera + " CI_TOE_"+%c+"_"+%s+"_2"
	endif
  next
next
  ...
  for %c {%list_com}
  for %s {%list_sec}
	if @isobject("CI_"+%c+"_"+%s+"_2") then
   	%export_opera = %export_opera + " CI_"+%c+"_"+%s+"_2"
	endif
  next
next

  for %s {%list_sec}
	if @isobject("PCID_cgas_"+%s+"_2") then
   	%export_opera = %export_opera + " PCID_cgas_"+%s+"_2"
	endif
  next

 %export_opera = %export_opera + " PM_2 PCHD_cgas_2 Un_2 UnR_2 empl_2 f_l_2 f_k_2 f_e_2 f_mat_2 PCH_2 GDP_2 EMS_2 EMS_Y_CO2_2 EMS_CI_CO2_2 EMS_MAT_CO2_2 EMS_CH_CO2_2 qd_cgas_2 GOS_2 WAGES_2 RSC_2 NTAXS_2 NTAXC_2 pcgas PCH_cgas_2 PCHD_cgas_2 PCHM_cgas_2 PCH_cele_2 PCHD_cele_2 PCHM_cele_2  CH_cgas_2 CHD_cgas_2 CHM_cgas_2 CH_cele_2 CHD_cele_2 CHM_cele_2"

  ' Create data group sheets 
    group shock {%export_opera}

  ' Export to excel  
    call savetoexcel("shock", "export_opera.xlsx", %scenario_name, "YES")


  else

  if %scenario_name = "scenario_ADAPT_cap_eff" then
  ...
  for %c {%list_com}
	if @isobject("pch_"+%c+"_2") then
   	%export_opera = %export_opera + " pch_"+%c+"_2"
	endif
  next

  for %c {%list_com}
	if @isobject("ch_"+%c+"_2") then
   	%export_opera = %export_opera + " ch_"+%c+"_2"
	endif
  next
  ...
  for %c {%list_com}
	if @isobject("pyqs_"+%c+"_2") then
   	%export_opera = %export_opera + " pyqs_"+%c+"_2"
	endif
  next

  for %c {%list_com}
	if @isobject("ci_toe_"+%c+"_2") then
   	%export_opera = %export_opera + " ci_toe_"+%c+"_2"
	endif
  next

  for %c {%list_com}
	if @isobject("ch_toe_"+%c+"_2") then
   	%export_opera = %export_opera + " ch_toe_"+%c+"_2"
	endif
  next

  for %s {%list_sec}
	if @isobject("EMS_CI_CO2_"+%s+"_2") then
   	%export_opera = %export_opera + " EMS_CI_CO2_"+%s+"_2"
	endif
  next
...
' Send results to Excel 
  ...
  'Output for main SHOCK_DEV variables
    
    %shock_dev = "100*(GDP_2/GDP_0-1) 100*(CH_2/CH_0-1) 100*(I_2/I_0-1) 100*(X_2/X_0-1) 100*(M_2/M_0-1) 100*((DISPINC_AT_VAL_2/PCH_2)/(DISPINC_AT_VAL_0/PCH_0)-1) 100*(RSAV_H_VAL_2-RSAV_H_VAL_0) 100*(PCH_2/PCH_0-1) 100*(PY_2/PY_0-1)  100*(PVA_2/PVA_0-1) 100*(PCI_2/PCI_0-1) 100*(PX_2/PX_0-1) 100*(PM_2/PM_0-1) 100*(W_2/W_0-1) 100*((C_L_2/PVA_2)/(C_L_0/PVA_0)-1) (F_L_2-F_L_0) 100*(UnR_2-UnR_0) 100*(RBal_Trade_VAL_2-RBal_Trade_VAL_0) 100*(RBal_G_Prim_VAL_2-RBal_G_Prim_VAL_0) 100*(RDEBT_G_VAL_2-RDEBT_G_VAL_0) 100*(CH_0+G_0)/GDP_0*((CH_2+G_2)/(CH_0+G_0)-1) 100*I_0/GDP_0*(I_2/I_0-1) 100*(X_0-M_0)/GDP_0*((X_2-M_2)/(X_0-M_0)-1) 100*DS_0/GDP_0*(DS_2/DS_0-1) "
    ' Can be added later when available in the calibration: 100*(EMS_CO2_2/EMS_CO2_0-1) 

  ' Concatenation des strings
   for %s {%list_sec}
   if @isobject("VA_"+%s+"_0") = 1 then
      %shock_dev = %shock_dev + " (VA_"+%s+"_2/VA_"+%s+"_0-1)"
   endif
   next
...
'output for main BASELINE variables
    ...
    ' Can be added later when available in the calibration: ' EMS_CO2_0 EMS_CH_CO2_0 EMS_CI_CO2_0
...
'' Can be added later when available in the calibration: 
''   for %c {%list_com}
...
' Output for main SHOCK variables
    ...
    ' EMS_CO2_2 EMS_CH_CO2_2 EMS_CI_CO2_2
...
' Can be added later when available in the calibration: 
''   for %c {%list_com}
''   if @isobject("EMS_"+%c+"_2") = 1 then
''     %shock = %shock + " EMS_"+%c+"_2"
''   endif
''   next

''   for %c {%list_com}
''   if @isobject("EMS_CO2_"+%c+"_2") = 1 then
''     %shock = %shock + " EMS_CO2_"+%c+"_2"
''   endif
''   next
...
' Create data group sheets 
    ...
    group Shock {%shock}  
call savetoexcel("Shock_dev Baseline Shock", "Result_NLD.xlsx", %scenario_name, "YES")
...
subroutine savetoexcel(string %groupresult, string %filemane, string %scenario_name,  string %duplicate)
  ...
  if %duplicate = "YES" then
    ...
    wfsave(type=excelxml, mode=update) {%pathfile2b} range={%sheet}!b1 byrow @keep {%sheet} @smpl {%baseyear} @last
endif
  next
endsub
' ============================================================================
